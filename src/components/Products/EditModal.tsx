...
const EditModal = ({ closeModal, product }: IProps) => {
    ...
    useEffect(() => {
        setValue('writer',desc.writer);
        setValue('aboutWriter',desc.aboutWriter);
        setValue('bookLag',desc.bookLag);
        setValue('shortSummery',desc.shortSummery);
        // setValue('longSummery',desc.longSummery);
        setValue('maxAge',Number(desc.maxAge));
        setValue('minAge',Number(desc.minAge));
        setValue('pages',Number(desc.pages));
        setValue('releaseData',new Date(desc.releaseData));
        setValue('translator',desc.translator);
                                ...
                                {imagesURL.map((imageURL:any, index:number) =>
                                    <div className={`mask mask-squircle relative  ${selectedImage === index && 'scale-110 transition-transform '}`}
                                        ...
                                        <div className='w-16 h-16 bg-base-300 flex items-center justify-center cursor-pointer'>
                                            {imagesFileRef.current?.files?.length ||  imagesURL ?
